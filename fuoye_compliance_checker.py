...
class FUOYEComplianceAgent:
    def __init__(self, doc_path, seminar_type="proposal", degree_type="phd"):
        ...
        self.report = {
            ...
            "passed": [],
            "warnings": [],  # Includes advisories and Critical Warnings (e.g. margins) - excluded from score
            "critical_violations": [], # Structural violations - included in score
    ...
    def run_full_audit(self):
        ...
        self._audit_referencing_and_authenticity()
        return self.report

    def _audit_margins(self):
        """
        Audits page margins against 3.125cm (1.25 in) left and 2.5cm (1.0 in) top/right/bottom.
        Margin issues are classified as 'Critical Warning' (warnings) so they do NOT affect score.
        """
        for idx, sec in enumerate(self.doc.sections):
            ...
            if not margin_issues:
                ...
                self.report["warnings"].append(
                    f"Critical Warning - Margin Violation in Section {idx+1}: " + ", ".join(margin_issues)
                )
    ...
    def _audit_referencing_and_authenticity(self):
        """
        Audits APA referencing, cross-matches in-text citations against the reference list,
        and generates a separate Reference Authenticity & Integrity Report.
        """
        ...
        # Extract in-text citations e.g. (Author, Year) or Author (Year)
        ...
        if total_citations > 0:
            self.report["passed"].append(f"In-Text Citations: Detected {total_citations} unique APA formatted citation(s).")
        else:
        ...
        self.report["reference_authenticity"] = {
    ...
    def print_summary(self):
        ...
        num_crit = len(self.report["critical_violations"])
        total_eval = num_pass + num_crit
        official_score = round((num_pass / total_eval) * 100) if total_eval > 0 else 100
        ...
        print(f"  OFFICIAL COMPLIANCE SCORE: {official_score}%")
        print("="*65 + "\n")
        ...
        if self.report["warnings"]:
            print("\n⚠️ WARNINGS & ADVISORIES (Indicated for Guidance - Excluded from Score):")
        ...
        if self.report["critical_violations"]:
            ...
            print("\n🎉 CONGRATULATIONS: No Critical Structural Violations Detected!")

        # Separate Reference Authenticity Report
        ref_auth = self.report.get("reference_authenticity", {})
        if ref_auth:
            print("\n" + "-"*65)
            print("  🔍 SEPARATE AUDIT: REFERENCE AUTHENTICITY & INTEGRITY REPORT")
            print(f"  Authenticity Status: {ref_auth.get('status', 'N/A')}")
            print(f"  Authenticity Index:  {ref_auth.get('authenticity_index', 0)}%")
            print(f"  Total In-Text Citations Scanned: {ref_auth.get('total_in_text_citations', 0)}")
            print(f"  Total Reference Entries Found:  {ref_auth.get('total_ref_entries', 0)}")
            
            unmatched = ref_auth.get("unmatched_citations", [])
            if unmatched:
                print(f"  ⚠️ Unmatched In-Text Citations (Missing from References): {', '.join(unmatched[:5])}")
            else:
                print("  ✔ All in-text citations were successfully matched to Reference List entries.")
            print("-"*65 + "\n")
