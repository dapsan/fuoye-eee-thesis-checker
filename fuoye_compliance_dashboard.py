...
class FUOYEManuscriptAuditor:
    ...
    def _check_title_and_abstract(self):
        ...
        # Check title length (First page title heading)
    ...
    def _check_citations(self):
        ...
        apa_citations = re.findall(r'\([A-Z][a-z]+(?:\s+(?:&|and)\s+[A-Z][a-z]+)?,\s+\d{4}\)', full_text)
    ...
    **Required Manuscript Specs:**
    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 Run Compliance Audit", type="primary"):
            with st.spinner("Analyzing manuscript typography, margins, structure, and captions..."):
                try:
                    ...
                    total_checks = num_pass + num_warn + num_fail
                    score = int((num_pass / total_checks) * 100) if total_checks > 0 else 100

                    # Metrics Cards
                    col1, col2, col3, col4 = st.columns(4)
                    col1.metric("Compliance Score", f"{score}%")
                    col2.metric("Passed Checks", num_pass, delta=f"{num_pass} passed", delta_color="normal")
                    col3.metric("Advisories / Warnings", num_warn, delta=f"{num_warn} warnings", delta_color="off")
                    ...
                    # Detailed Breakdowns
                    if num_fail > 0:
                        st.error(f"❌ **CRITICAL VIOLATIONS ({num_fail})** - Must be fixed before seminar presentation:")
                        for item in results["critical"]:
                            st.write(f"- 🔴 {item}")

                    if num_warn > 0:
                        st.warning(f"⚠️ **ADVISORIES & RECOMMENDATIONS ({num_warn})**:")
...
=================== CRITICAL VIOLATIONS ===================
""" + ("\n".join([f"- {x}" for x in results["critical"]]) if results["critical"] else "None!") + """

=================== ADVISORIES & WARNINGS ===================
""" + ("\n".join([f"- {x}" for x in results["warnings"]]) if results["warnings"] else "None!") + """
